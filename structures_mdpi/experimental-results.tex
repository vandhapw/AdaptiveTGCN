...
\caption{Ablation summary on the held-out test split: the auxiliary objectives (Configuration 3) deliver the largest MSE reduction, which AUAA (Configuration 4) and the full integration (Configuration 5) preserve.}
...
\input{tables/skab_singlesensor}
...
\caption{Anomaly detection diagnostics on the held-out test split for (a) Base TCN and (b) Full Enhanced. Each panel shows the anomaly-score timeline with the validation-calibrated $\tau_{95}$ (dashed red) and flagged windows, the score distribution, the per-sensor attribution, and the error--uncertainty relationship.}
...
\input{tables/skab_msensitivity}
